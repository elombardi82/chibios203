 ...
 * @brief   Micron MT25Q serial flash driver code.
 *
 * @addtogroup MICRON_MT25Q
 * @{
 */
...
#if MT25Q_USE_SUB_SECTORS == TRUE
#define SECTOR_SIZE                         0x00001000U
#define CMD_SECTOR_ERASE                    MT25Q_CMD_SUBSECTOR_ERASE
#else
#define SECTOR_SIZE                         0x00010000U
#define CMD_SECTOR_ERASE                    MT25Q_CMD_SECTOR_ERASE
#endif
...
const wspi_command_t snor_memmap_read = {
  ...
  .addr             = 0,
  .dummy            = MT25Q_READ_DUMMY_CYCLES - 2,
  .cfg              = WSPI_CFG_ADDR_SIZE_24 |
#if MT25Q_BUS_MODE == MT25Q_BUS_MODE_WSPI1L
                      WSPI_CFG_CMD_MODE_ONE_LINE |
                      WSPI_CFG_ADDR_MODE_ONE_LINE |
                      WSPI_CFG_DATA_MODE_ONE_LINE |
#elif MT25Q_BUS_MODE == MT25Q_BUS_MODE_WSPI2L
...
static const wspi_command_t mt25q_cmd_read_id = {
  ...
  .cfg              = 0U |
                      ...
                      WSPI_CFG_CMD_MODE_TWO_LINES |
                      WSPI_CFG_DATA_MODE_TWO_LINES,
#elif MT25Q_BUS_MODE == MT25Q_BUS_MODE_WSPI4L
                      WSPI_CFG_CMD_MODE_FOUR_LINES |
                      WSPI_CFG_DATA_MODE_FOUR_LINES,
#else
...
static const wspi_command_t mt25q_cmd_write_enable = {
...
#if MT25Q_BUS_MODE == MT25Q_BUS_MODE_WSPI1L
static const uint8_t mt25q_evconf_value[1] = {0xEF};
#elif MT25Q_BUS_MODE == MT25Q_BUS_MODE_WSPI2L
static const uint8_t mt25q_evconf_value[1] = {0xAF};
#else
...
static bool mt25q_find_id(const uint8_t *set, size_t size, uint8_t element) {
  size_t i;
...
static flash_error_t mt25q_poll_status(SNORDriver *devp) {
  uint8_t sts;

  do {
#if MT25Q_NICE_WAITING == TRUE
    osalThreadSleepMilliseconds(1);
#endif
    /* Read status command.*/
    bus_cmd_receive(devp->config->busp, MT25Q_CMD_READ_FLAG_STATUS_REGISTER,
                    1, &sts);
  } while ((sts & MT25Q_FLAGS_PROGRAM_ERASE) == 0U);

  /* Checking for errors.*/
  if ((sts & MT25Q_FLAGS_ALL_ERRORS) != 0U) {
    /* Clearing status register.*/
    bus_cmd(devp->config->busp, MT25Q_CMD_CLEAR_FLAG_STATUS_REGISTER);
...
#if (SNOR_BUS_DRIVER == SNOR_BUS_DRIVER_WSPI) || defined(__DOXYGEN__)
static void mt25q_reset_memory(SNORDriver *devp) {

  /* 1x MT25Q_CMD_RESET_ENABLE command.*/
  ...
  /* 4x MT25Q_CMD_RESET_MEMORY command.*/
...
static const uint8_t mt25q_manufacturer_ids[] = MT25Q_SUPPORTED_MANUFACTURE_IDS;
static const uint8_t mt25q_memory_type_ids[] = MT25Q_SUPPORTED_MEMORY_TYPE_IDS;
static const uint8_t mt25q_memory_capacities[] = MT25Q_SUPPORTED_MEMORY_CAPACITIES;
...
void snor_device_init(SNORDriver *devp) {
  ...
  /* Reading device ID.*/
  bus_cmd_receive(devp->config->busp, MT25Q_CMD_READ_ID,
  ...
  /* Attempting a reset of the device, it could be in an unexpected state
     because a CPU reset does not reset the memory too.*/
  mt25q_reset_memory(devp);

  /* Reading device ID and unique ID.*/
  wspiReceive(devp->config->busp, &mt25q_cmd_read_id,
              sizeof devp->device_id, devp->device_id);
#endif /* SNOR_BUS_DRIVER == SNOR_BUS_DRIVER_WSPI */

  /* Checking if the device is white listed.*/
  osalDbgAssert(mt25q_find_id(mt25q_manufacturer_ids,
                             sizeof mt25q_manufacturer_ids,
                             devp->device_id[0]),
                "invalid manufacturer id");
  osalDbgAssert(mt25q_find_id(mt25q_memory_type_ids,
                             sizeof mt25q_memory_type_ids,
                             devp->device_id[1]),
                "invalid memory type id");
  osalDbgAssert(mt25q_find_id(mt25q_memory_capacities,
                             sizeof mt25q_memory_capacities,
                             devp->device_id[2]),
                "invalid memory capacities");

#if (SNOR_BUS_DRIVER == SNOR_BUS_DRIVER_WSPI) && (MT25Q_SWITCH_WIDTH == TRUE)
  /* Setting up final bus width.*/
  wspiCommand(devp->config->busp, &mt25q_cmd_write_enable);
  wspiSend(devp->config->busp, &mt25q_cmd_write_evconf, 1, mt25q_evconf_value);

  {
    uint8_t id[20];

    /* Reading ID again for confirmation.*/
    bus_cmd_receive(devp->config->busp, MT25Q_CMD_MULTIPLE_IO_READ_ID, 3, id);
...
#if SNOR_BUS_DRIVER == SNOR_BUS_DRIVER_WSPI
  {
    ...
    bus_cmd(devp->config->busp, MT25Q_CMD_WRITE_ENABLE);
...
flash_error_t snor_device_read(SNORDriver *devp, flash_offset_t offset,
  ...
  bus_cmd_addr_dummy_receive(devp->config->busp, MT25Q_CMD_FAST_READ,
                             offset, MT25Q_READ_DUMMY_CYCLES, n, rp);
#else
  /* Normal read command in SPI mode.*/
  bus_cmd_addr_receive(devp->config->busp, MT25Q_CMD_READ,
                       offset, n, rp);
#endif
...
flash_error_t snor_device_program(SNORDriver *devp, flash_offset_t offset,
  ...
  while (n > 0U) {
    ...
    bus_cmd_addr_send(devp->config->busp, MT25Q_CMD_PAGE_PROGRAM, offset,
                      chunk, pp);

    /* Wait for status and check errors.*/
    err = mt25q_poll_status(devp);
...
flash_error_t snor_device_start_erase_all(SNORDriver *devp) {
  ...
  /* Bulk erase command.*/
  bus_cmd(devp->config->busp, MT25Q_CMD_BULK_ERASE);
...
flash_error_t snor_device_start_erase_sector(SNORDriver *devp,
  ...
  bus_cmd_addr(devp->config->busp, MT25Q_CMD_SECTOR_ERASE, offset);
...
flash_error_t snor_device_verify_erase(SNORDriver *devp,
                                       flash_sector_t sector) {
  uint8_t cmpbuf[MT25Q_COMPARE_BUFFER_SIZE];
  ...
  while (n > 0U) {
...
#if SNOR_BUS_DRIVER == SNOR_BUS_DRIVER_WSPI
   bus_cmd_addr_dummy_receive(devp->config->busp, MT25Q_CMD_FAST_READ,
                              offset, MT25Q_READ_DUMMY_CYCLES,
   ...
   bus_cmd_addr_receive(devp->config->busp, MT25Q_CMD_READ,
    ...
    for (p = cmpbuf; p < &cmpbuf[MT25Q_COMPARE_BUFFER_SIZE]; p++) {
      if (*p != 0xFFU) {
        /* Ready state again.*/
...
flash_error_t snor_device_query_erase(SNORDriver *devp, uint32_t *msec) {
  ...
  if (((sts & MT25Q_FLAGS_PROGRAM_ERASE) == 0U) ||
      ((sts & MT25Q_FLAGS_ERASE_SUSPEND) != 0U)) {
...
void snor_activate_xip(SNORDriver *devp) {
  static const uint8_t flash_status_xip[1] = {
    (MT25Q_READ_DUMMY_CYCLES << 4U) | 0x07U
  };
...
void snor_reset_xip(SNORDriver *devp) {
  ...
  cmd.alt   = 0xFFU;
  cmd.addr  = 0U;
  cmd.dummy = MT25Q_READ_DUMMY_CYCLES - 2U;
